function feat=featuresig(sig,label1)
    ...
    for i=1:x
        ...
        %    clear l;
        %for j=1:14
         %   feat{i,j}=mean(abs(sig{i}(:,j)));%mean
        %end
        
        %for j=15:28
         %  feat{i,j}=rms(sig{i}(:,j-14));%rms
        %end
        %for j=43:56
         %   feat{i,j}=var(sig{i}(:,j-42));%variance
        %end
       % for j=29:42
        %   feat{i,j}=trapz(t,(abs(sig{i}(:,j-28)).^2));%square integral
        %end
        %for j=71:84
        %    feat{i,j}=sum(diff(sign(diff(sig{i}(:,j-70))))~=0);%slope sign change
       % end
        %for j=85:98
      ...
      % end
        ...
        %for j=113:196
         %   feat{i,j}=temp(j-112);%autoregression
        %end
        %for j=197:210
         %   feat{i,j}=getwlfeat(sig{i}(:,j-196));%waveform length
        %end
        %for j=211:224
           % F = fft(sig{i}(:,j-210));%wavelet energy
          %  pow = F.*conj(F);
         %   feat{i,j} = sum(pow);
        %end
        %clear temp
        ...
        %for j=225:308
         %   feat{i,j}=temp(j-224);%independent component analysis
        %end
        %clear temp
         for j=1:14
        [p,f]=psd(sig{i}(:,j));% mean power frequency
	...
	%feat{i,j} = std(diff(diff(sig{i}(:,j-350))))./std(diff(sig{i}(:,j-350)))./mobility;
     % end
      %for j=365:378
       %  feat{i,j}=fractal((sig{i}(:,j-364))',16);%fractal dimension
      %end
      %for j=379:392
       %  feat{i,j}=Katz_FD((sig{i}(:,j-378))');
      %end
                                              %    for j=211:224
...
%calculation of sppectral edge frequency
 	  %    for j=351:363
 		%Yaq = abs(fft(sig{i}));                  % take FFT of each channel
